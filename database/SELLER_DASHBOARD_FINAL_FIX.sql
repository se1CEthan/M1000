-- 🚀 SELLER DASHBOARD FINAL FIX - Simple and error-free
-- This creates all seller dashboard functionality without conflicts

-- 1. Drop existing functions if they exist (to avoid conflicts)
DROP FUNCTION IF EXISTS generate_order_number();
DROP FUNCTION IF EXISTS update_seller_earnings();
DROP FUNCTION IF EXISTS increment_product_view(uuid);
DROP FUNCTION IF EXISTS increment_product_download(uuid);
DROP FUNCTION IF EXISTS update_product_rating(uuid);

-- 2. Ensure seller_pending_balances table exists
...
-- 3. Ensure seller_crypto_wallets table exists
...
-- 5. Ensure seller_analytics table exists
...
-- 6. Create seller_notifications table
...
-- 7. Create seller_settings table
...
-- 8. Add missing columns to existing tables (ignore errors if columns exist)
DO $$ 
BEGIN
...
-- 9. Create indexes for performance (ignore errors if they exist)
...
-- 11. Create function to update seller earnings
...
-- 12. Create triggers (drop existing first)
...
-- 17. Create default seller settings for existing sellers (simple INSERT)
DO $$
DECLARE
...
BEGIN
    ...
    LOOP
        ...
        VALUES (seller_record.user_id)
        ON CONFLICT (seller_id) DO NOTHING;
    END LOOP;
END $$;

-- 18. Create sample data for testing (simple approach)
DO $$
DECLARE
    admin_user_id uuid;
BEGIN
...
SELECT 
  ...
  COALESCE(p.seller_rating, 0) as seller_rating,
  COALESCE(p.seller_level, 'bronze') as seller_level,
...
-- Success message
SELECT '🎉 SELLER DASHBOARD SETUP COMPLETE! All features ready for production use.' as result;