...
serve(async (req) => {
  ...
  try {
    console.log('🔔 PesaPal IPN received at /pesapal/ipn')
    
    // Parse the IPN data from URL parameters
...
async function checkPesaPalPaymentStatus(trackingId: string): Promise<{
  ...
  try {
    ...
    const response = await fetch(
      `${PESAPAL_CONFIG.BASE_URL}/api/Transactions/GetTransactionStatus?orderTrackingId=${trackingId}`,
      {
        method: 'GET',
        headers: {
