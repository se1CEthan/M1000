...
interface SellerVerificationReviewProps {
  onApplicationsChange?: () => void;
}

export function SellerVerificationReview({ onApplicationsChange }: SellerVerificationReviewProps) {
      ...
      render: (app) => (
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          ...
          <div className="min-w-0 flex-1">
            <div className="font-medium text-sm sm:text-base truncate">{app.full_name}</div>
            <div className="text-xs sm:text-sm text-muted-foreground truncate">{app.business_name || 'Individual'}</div>
          </div>
        </div>
      )
    },
    {
      ...
      render: (app) => (
        <Badge variant="outline" className="capitalize text-xs">
          <span className="hidden sm:inline">{app.business_type}</span>
          <span className="sm:hidden">{app.business_type.slice(0, 3)}</span>
        </Badge>
      )
    },
    {
    ...
    },
    {
      ...
      render: (app) => (
        <span className="text-sm font-medium">
          <span className="hidden sm:inline">${app.expected_monthly_sales?.toFixed(0) || '0'}/mo</span>
          <span className="sm:hidden">${app.expected_monthly_sales?.toFixed(0) || '0'}</span>
        </span>
      )
    },
    {
      ...
      render: (app) => (
        <Badge className={`text-xs ${getStatusColor(app.status)}`}>
          ...
          <span className="hidden sm:inline">{app.status?.toUpperCase() || 'PENDING'}</span>
          <span className="sm:hidden">{app.status?.slice(0, 3).toUpperCase() || 'PEN'}</span>
        </Badge>
      )
    },
    {
      ...
      render: (app) => (
        <span className="text-xs sm:text-sm text-muted-foreground">
          ...
          <span className="sm:hidden">{new Date(app.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
        </span>
      )
    },
    {
    ...
    return (
      <div className="space-y-4 sm:space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
          <div className="min-w-0 flex-1">
            <h2 className="text-xl sm:text-2xl font-bold">Seller Application Review</h2>
            <p className="text-sm text-muted-foreground">Review and approve seller verification</p>
          </div>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium text-sm sm:text-base">{app.full_name}</p>
                  </div>
                  <div>
                    <Label className="text-xs sm:text-sm font-medium text-muted-foreground">Phone</Label>
                    <p className="font-medium text-sm sm:text-base">{app.phone_number}</p>
                  </div>
                  <div>
                    <Label className="text-xs sm:text-sm font-medium text-muted-foreground">Date of Birth</Label>
                    <p className="font-medium text-sm sm:text-base">{new Date(app.date_of_birth).toLocaleDateString()}</p>
                  </div>
                  <div>
                    <Label className="text-xs sm:text-sm font-medium text-muted-foreground">Business Type</Label>
                    <p className="font-medium text-sm sm:text-base capitalize">{app.business_type}</p>
                  </div>
                </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {app.business_name && (
                    <div>
                      <Label className="text-xs sm:text-sm font-medium text-muted-foreground">Business Name</Label>
                      <p className="font-medium text-sm sm:text-base">{app.business_name}</p>
                    </div>
                  )}
                  {app.business_registration && (
                    <div>
                      <Label className="text-xs sm:text-sm font-medium text-muted-foreground">Registration Number</Label>
                      <p className="font-medium text-sm sm:text-base">{app.business_registration}</p>
                    </div>
                  )}
                  {app.tax_id && (
                    <div>
                      <Label className="text-xs sm:text-sm font-medium text-muted-foreground">Tax ID</Label>
                      <p className="font-medium text-sm sm:text-base">{app.tax_id}</p>
                    </div>
                  )}
                  <div>
                    <Label className="text-xs sm:text-sm font-medium text-muted-foreground">Expected Monthly Sales</Label>
                    <p className="font-medium text-sm sm:text-base">${app.expected_monthly_sales?.toFixed(0) || '0'}</p>
                  </div>
                </div>

                <div>
                  <Label className="text-xs sm:text-sm font-medium text-muted-foreground">Selling Reason</Label>
                  <p className="font-medium text-sm sm:text-base mt-1 p-3 bg-muted rounded-lg">{app.selling_reason}</p>
                </div>

                <div>
                  <Label className="text-xs sm:text-sm font-medium text-muted-foreground">Product Categories</Label>
          ...
          <div className="space-y-4 sm:space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="text-center">
                  <Badge className={`${getStatusColor(app.status)} text-sm sm:text-lg px-4 py-2`}>
                    {getStatusIcon(app.status)}
                ...
                {app.admin_notes && (
                  <div>
                    <Label className="text-xs sm:text-sm font-medium text-muted-foreground">Admin Notes</Label>
                    <p className="text-xs sm:text-sm mt-1 p-2 bg-muted rounded">{app.admin_notes}</p>
                  </div>
                )}
              </CardContent>
            </Card>
            ...
            {app.status === 'pending' && (
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div>
                    <Label htmlFor="admin-notes" className="text-xs sm:text-sm">Admin Notes (Optional)</Label>
                    <Textarea
                      id="admin-notes"
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        <div className="min-w-0 flex-1">
          <h2 className="text-xl sm:text-2xl font-bold">Seller Verification Applications</h2>
          <p className="text-sm text-muted-foreground">Review and approve seller applications</p>
        </div>
        <Button onClick={fetchApplications} variant="outline" disabled={loading} className="w-full sm:w-auto">
