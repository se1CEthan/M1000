...
export default function Auth() {
  ...
  useEffect(() => {
    if (user && redirectParam === 'freelancing') {
      navigate('/freelancing-role-selection');
    }
  }, [user, redirectParam, navigate]);

  const handleOAuthSignIn = async (provider: string, signInFunction: () => Promise<{ error: Error | null }>) => {
    setLoading(provider);
    try {
  ...
  const authProviders = [
    {
      ...
      action: () => handleOAuthSignIn('google', signInWithGoogle),
    ...
    },
    {
      ...
      action: () => handleOAuthSignIn('github', signInWithGitHub),
      ...
      <Card className="relative w-full max-w-md border-border/50 bg-card/80 backdrop-blur">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-2xl">
            {redirectParam === 'freelancing' ? 'Join Seltech Freelancing' : 'Welcome to Seltech'}
          </CardTitle>
          <CardDescription>
            {redirectParam === 'freelancing' 
              ? 'Sign in to start your freelancing journey or find talented professionals'
              : 'Sign in with your preferred platform to access the developer marketplace'
            }
          </CardDescription>
        </CardHeader>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center justify-center mb-6">
            <Badge variant="secondary" className="text-xs">
              🔒 Secure OAuth Authentication Only
            </Badge>
          </div>
          ...
          <div className="mt-6 p-4 bg-muted/50 rounded-lg border border-border/50">
            <h4 className="text-sm font-medium text-foreground mb-2">🛡️ Enhanced Security</h4>
            <p className="text-xs text-muted-foreground">
              We use OAuth authentication for maximum security. Your credentials are never stored on our servers.
            </p>
          </div>
